...
bool write_data(const std::string& file_name,
                const std::map< std::string, Student* >& alphabetical_order) {
    std::ofstream file_object(file_name);
    if (!file_object) {
        return  false;
    }

    for (const auto& pair : alphabetical_order) {
...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            std::string student_number(parts[1]);
            if (student_numbers.count(student_number) == 0) {
                ...
                if (is_valid_phone_number((new_phone_number))){
                    Student* student = student_numbers[student_number];
